...
\begin{tikzpicture}[
  ...
  \path[use as bounding box] (-1.8,1.5) rectangle (17.8,-5.9);
  ...
  \draw[flow] (proj) -- (cons);
  \draw[flow] (proj.south) -- ++(0,-.30) -| (vis.south);

  \node[smallbox, fill=target] (corrupt) at (12.0,-4.65) {first-global $z$\\span mask + noise};
  ...
  \draw[flow] (z.west) -- ++(-.35,0) |- (corrupt.east);
  ...
  \node[smallbox, fill=evaluation] (probes) at (16.0,-2.25) {frozen-feature\\probes};
  \draw[evalflow] (z) -- (probes);

  \begin{scope}[on background layer]
    \node[group, fit=(proj)(cons)(vis), label={[font=\sffamily\scriptsize,ink]above:parallel representation objectives on $p$}] {};
